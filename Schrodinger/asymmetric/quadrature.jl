...
setprecision(prec÷2)
W̄ = enlarge.(sqrt.(sqrt.(W/interval(4))))
V4 = zeros(Interval{BigFloat}, (N, sum(Ms.+1)))
# change of variables
Yrig4 = enlarge.(Xrig ./interval(2));
...
# quick tests
if in_interval(55//512//2, sum((V4[:,indices[2]+1]/sqrt(interval(BigFloat,2))).^4))
...
indices = big.([1; cumsum(Ms.+1).+1])
setprecision(prec÷2)
W̄ = enlarge.(cbrt.(sqrt.(W/interval(6))))
# Vandermonde matrix is with respect to normalised Laguerre polynomials
V6 = zeros(Interval{BigFloat}, (N, sum(Ms.+1)))
# change of variables
Yrig6 = enlarge.(Xrig ./interval(3));
...
if in_interval(22540//1594323//2, sum((V6[:,indices[2]+1]/sqrt(interval(BigFloat,2))).^6))
    println("integral verification succeeded")
else
    println("integral verification failed")
end
