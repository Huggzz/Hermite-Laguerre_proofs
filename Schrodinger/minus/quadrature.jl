...
# quick tests
if in_interval(29//243, sum(V6[:,2].^6))
    println("integral ∫(1-z)^6 exp(-3z) dz = 29/243 checked")
else
...
# weights for the Gaussian-Laguerre quadrature
W = Xrig ./((laguerre(N+1,Xrig)).^interval(2))./(interval(N+1)^interval(2))
W̄ = (sqrt.(sqrt.(W/interval(2))))

setprecision(prec ÷ 2)
# Vandermonde matrix is with respect to normalised Laguerre polynomials
V4 = zeros(Interval{BigFloat}, (N, n+1))
# change of variables
Yrig4 = Xrig ./interval(2);
...
# quick tests
if in_interval(1//4, sum(V4[:,2].^4))
    println("integral ∫(1-z)^4 exp(-2z) dz=1/84 checked")
else
    println("integral verification failed")
end
